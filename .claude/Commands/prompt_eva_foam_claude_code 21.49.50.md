...
> Mỗi BƯỚC có thể là 1 session riêng — xem hướng dẫn "Khi mở session mới"
> ở cuối mỗi BƯỚC.
...
═══════════════════════════════════════════════════════
MODEL & EFFORT — ĐỌC TRƯỚC KHI LÀM BẤT CỨ THỨ GÌ
═══════════════════════════════════════════════════════

Áp dụng đúng model/effort cho từng bước theo bảng sau:

  BƯỚC 1 (đọc PDF, lập bảng 122 SKU, dịch tên)  → model cao nhất, effort MAX
  BƯỚC 2 (populate JS từ bảng đã duyệt)           → effort High
  BƯỚC 3 (crop ảnh layout bằng Pillow, code thuần) → effort Medium
  BƯỚC 4a (crop nhóm S từ PDF)                    → model cao nhất, effort MAX
  BƯỚC 4b (crop nhóm M từ PDF)                    → model cao nhất, effort MAX
  BƯỚC 4c (crop nhóm L từ PDF)                    → model cao nhất, effort MAX
  BƯỚC 4d (crop nhóm XL từ PDF)                   → model cao nhất, effort MAX
  BƯỚC 5 (build HTML + sửa nav)                   → effort High

Lý do: các BƯỚC đọc PDF có rủi ro bịa mã/specs cao nhất — phải dùng
model mạnh nhất + effort MAX để đảm bảo không fabricate dữ liệu.
...
NGUỒN A — 3 ảnh layout lớn (hero + 2 block giới thiệu):
  ...
  → Crop bằng Python + Pillow (kiểm tra Pillow trước: `python3 -c "from PIL
    import Image"` — nếu lỗi thì `pip3 install Pillow` trước khi crop).
  ...
  A1 → eva-foam-hero.png
       ...
       khi xuất hiện nền trắng nav bar).
  ...
  Workflow crop: in kích thước ảnh gốc (width×height) → DỪNG cho tôi
  xác nhận tọa độ → crop → báo kích thước output. (Xem BƯỚC 3.)

NGUỒN B — Ảnh từng bộ foam sản phẩm:
  Pipeline chuẩn (CHỈ ĐỌC):
...
PHẦN TRÊN (above the fold + giới thiệu):
  ...
  5. Hàng 4 thẻ size (S / M / L / XL), mỗi thẻ:
     - Ảnh đại diện = ảnh của SKU tiêu biểu đã crop trong BƯỚC 4
       (chọn bộ nhiều món/ảnh đẹp nhất của size đó, dùng LẠI ảnh đã crop
        — KHÔNG crop ảnh mới — ghi rõ SKU nào được chọn)
...
════ BƯỚC 1 — LẬP BẢNG ════
Model cao nhất · Effort MAX

1a. Kiểm tra branch: git branch --show-current → phải là claude/epic-knuth-rtyfc5.
    Branch lạ → báo ngay, KHÔNG làm gì thêm.

1b. Đọc catalog trang 66–75 qua git show. Lập bảng TẤT CẢ SKU:
    | size | sku | name_en | pieces | name_vi (đề xuất) | trang | nhãn phụ | (?) |
    In tổng count từng size + tổng. So sánh với ~122.
...
[ Nếu mở session mới cho BƯỚC 2 trở đi: dán lại đoạn đầu
  "Repo Sonic-web... Đọc CLAUDE.md" + ghi rõ "Tiếp tục từ BƯỚC X,
  bảng/data đã được duyệt ở session trước." ]

════ BƯỚC 2 — TẠO data/eva-foam.js ════
Effort High
(Chỉ làm sau khi tôi duyệt bảng BƯỚC 1)
...
Effort Medium
(Có thể chạy song song hoặc sau BƯỚC 2 — không phụ thuộc nhau)

3a. Kiểm tra Pillow: python3 -c "from PIL import Image; print('OK')"
    Nếu lỗi: pip3 install Pillow --break-system-packages

3b. Đọc file /Users/lucatxtruong/Downloads/screenshot EVA foam.png
    In: kích thước (width × height px) + mô tả ngắn vùng thấy được.
    DỪNG — báo tôi xác nhận trước khi crop.

3c. Sau khi tôi xác nhận tọa độ, crop 3 ảnh A1/A2/A3 như mô tả ở trên.
    Lưu vào assets/products/eva-foam/.
    Báo kích thước từng file output (px + KB).
    git add assets/products/eva-foam/eva-foam-*.png
    DỪNG.

════ BƯỚC 4 — CROP ẢNH SẢN PHẨM (Nguồn B) ════
Model cao nhất · Effort MAX · CHẠY TỪNG NHÓM = TỪNG SESSION RIÊNG

4a. Nhóm S (catalog trang 68–71) — model cao nhất, effort MAX:
    - Crop từng SKU size S.
    - Chọn 1 SKU đại diện size S (bộ nhiều món nhất / ảnh rõ đẹp nhất).
    - Cập nhật trường image trong data/eva-foam.js cho toàn bộ nhóm S.
    - Log: đã crop / bỏ qua (verified:false).
    - git add assets/products/eva-foam/ && git diff --stat
    DỪNG.
    [ Session tiếp: "Repo Sonic-web... Đọc CLAUDE.md. Tiếp BƯỚC 4b nhóm M.
      eva-foam.js đã có nhóm S. Dùng model cao nhất, effort MAX." ]

4b. Nhóm M (trang 72–73) — model cao nhất, effort MAX:
    Crop + chọn SKU đại diện M + cập nhật JS + git diff. DỪNG.

4c. Nhóm L (trang 74) — model cao nhất, effort MAX:
    Crop + chọn SKU đại diện L + cập nhật JS + git diff. DỪNG.

4d. Nhóm XL (trang 75) — model cao nhất, effort MAX:
    Crop + chọn SKU đại diện XL + cập nhật JS + git diff.
    Sau 4d: in bảng tổng kết 4 SKU đại diện được chọn cho 4 thẻ size.
    DỪNG.
...
5c. Kiểm tra thủ công:
    - Mở eva-foam.html bằng file:// trong browser.
    - Filter [S] / [M] / [L] / [XL] hoạt động đúng (ẩn/hiện card).
    - Click thẻ size ở phần trên → cuộn xuống + filter đúng size.
    - Nav dropdown "Hệ EVA Foam Sonic" click được, vào đúng trang.
